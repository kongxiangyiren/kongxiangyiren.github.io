...
const RESOLVED_POST_BODIES = '\0' + VIRTUAL_POST_BODIES_MODULE;
...
/**
 * 预渲染路由清单（纯函数，方便单测与人工核对）。
 *
 * 覆盖：首页、四个总览页、关于 / 友链、每篇文章、每个标签 / 分类，
 * 外加 `404.html` —— 它是给 nginx `error_page 404 /404.html` 用的 404 页面产物
 * （静态托管无法在「路径不存在」时返回状态码，只能靠这个文件）。
 *
 * 刻意列全而不依赖 `crawlLinks`：爬虫只跟随渲染出来的 `<a href>`，而分页、侧栏、
 * 导航都可能漏；漏掉的页面**不会报错**，会静默地没有产物。爬虫仍开着，作为额外补充。
 */
export function buildPrerenderRoutes(
  ...
  return [
    ...
    ...taxonomy.tags.map(tag => `/tags/${tag.name}`),
    ...taxonomy.categories.map(category => `/categories/${category.name}`)
  ];
}
...
interface BuiltContent {
  ...
  /** slug → 正文。**只给服务端预渲染用**（见 src/api/post.ts 的 SSR 分支） */
  postBodies: Record<string, BlogPostBody>;
}
...
async function buildContent(contentDir: string, base: string): Promise<BuiltContent> {
  ...
  const rendered: RenderedPost[] = pending.map(item => {
    ...
    return {
      ...
      description: toOptionalString(fm.description) ?? plainText.slice(0, DESCRIPTION_LIMIT).trim(),
  ...
  return { posts, taxonomy: buildTaxonomy(rendered), searchIndex, bodies, postBodies };
}
...
export function blogContent(options: BlogContentPluginOptions = {}): Plugin {
  ...
  return {
    ...
    resolveId(id) {
      ...
      if (id === VIRTUAL_POST_BODIES_MODULE) return RESOLVED_POST_BODIES;
      return null;
    },
    ...
    async load(id) {
      ...
      if (id === RESOLVED_POST_BODIES) {
        const { postBodies } = await getContent();
        /*
         * `moduleSideEffects: false`：这个模块只被 `import.meta.env.SSR` 为真的动态
         * import 引用，声明无副作用后，浏览器包里连它的壳都不会留下（回归红线见验收 6）。
         */
        return {
          code: `export const postBodies = ${serialize(postBodies)}\n`,
     ...
     * 预渲染路由清单与 HTML 注入。
     *
     * 为什么**不靠爬虫**：`crawlLinks` 只有等到页面渲染出来才能发现 `<a href>`，
     * 而分页按钮、导航、侧栏都是可变的 —— 漏一条链就是漏一个页面，而且**无声**
     * （`failOnError` 只能抓住「发现了但渲染失败」，抓不住「根本没发现」）。
     * 所以路由从构建期内容数据里显式列出来（文章 / 标签 / 分类都在这份数据里），
     * `crawlLinks` 仍开着作为补充（将来正文里出现站内链接时能自动覆盖）。
     */
    nitro: {
      name: 'blog:content-prerender',
      setup(nitro) {
        nitro.hooks.hook('prerender:routes', async routes => {
          const { posts, taxonomy } = await getContent();
          for (const route of buildPrerenderRoutes(posts, taxonomy)) {
            routes.add(route);
          }
        });
      }
